...
export default Ember.Service.extend({
  ...
  loadDictionary: function () {
    this.wordDict  = this.fileRead("dict.dat");
    this.wordIndex = this.fileRead("dict.idx");
  },
  ...
  makeHtml: function (entry, tdata) {
    ...
    for (i = 0; i < entry.data.length; ++i) {
      ...
      //HANZI
      k = "";
      if ("botht" == tdata.config.showhanzi || "boths" == tdata.config.showhanzi) {
        var first  = tdata.config.showhanzi == "botht" ? trad : simp;
        var second = tdata.config.showhanzi == "botht" ? simp : trad;
        ...
        var hanzi = tdata.config.showhanzi == "simp" ? simp : trad;
        if (tdata.config.docolors == "yes") {
          for (j = 0; j < pinyin.tones.length; j++) {
      ...
      //PINYIN
      k += '&#32;&#32; <span class="w-kana">';
      if ("tonenums" == tdata.config.pinyin) {
        k += pinyin.tonenums + '</span>';
      } else if ("zhuyin" == tdata.config.pinyin) {
